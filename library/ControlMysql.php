 ...
 */
class ControlMysql /*implements ControllDB*/ {
	const CLASS_NAME = 'ControllTable';
	...
	public function __construct($db_conf, $table_name = null, $is_debug = false) {
		$this->setDebug ( $is_debug );
		$this->_db_conf = $db_conf;
		$this->_mysql = new Mysql ( $db_conf );
		if (! empty ( $table_name ) && ! $this->setTableName ( $table_name )) {
			if (! $is_debug) {
				$this->echoErrorMessage ( $this->getErrorMessage () );
			} else {
				exit ();
			}
		}
	...
	private function _buildWhereCondition($where_arr) {
		...
		foreach ( $where_arr as $condition ) {
			...
			if ($condition ['operator'] == 'LIKE' || $condition ['operator'] == 'NOT LIKE') {
				$result .= $condition ['operator'] . ' "%' . addslashes ( $condition ['condition'] ) . '%"';
	 ...
	 * @return null
	 */
	private function _debugDump($data) {
		if ($this->getDebug ()) {
			Debug::debugDump ( $data );
			// var_dump ( $data );
		}
	}
	...
	protected function getCharFields() {
		...
		$result ['_set_up_completed_'] = true;
	 ...
	 * 设置当前错误信息
	 */
	protected function setErrorMessage($errMsg) {
		return Debug::debugDump ( $errMsg );
	}
	/**
	 * 获取当前错误信息
	 */
	public function getErrorMessage() {
		return Debug::getErrorMessage();
	}
	/**
	 * 输出错误信息并退出
	 */
	protected function echoErrorMessage($error_message) {
		return Debug::echoErrorMessage($error_message);
	}
	 ...
	 * @return array
	 */
	protected function getListForUpdate($count = 10, $page = 0, $fields = '*', $where_condition = null, $order_by = null, $group_by = null) {
		if (! is_numeric ( $count ) || ! is_numeric ( $page ) || empty ( $fields )) {
			$this->setErrorMessage ( self::PARAMS_ERROR_MESSAGE );
			return false;
		}
		...
		$sql = 'SELECT ' . $fields . ' FROM ' . $this->getTableName ( true ) . $this->_buildWhereCondition ( $where_condition ) . $this->_buildGroupBy ( $group_by ) . $this->_buildOrderBy ( $order_by ) . $limit.' FOR UPDATE';

		RunTime::start ( "ControllMysql" );
		$result = $this->_mysql->performSelectForUpdate ( $sql );
		RunTime::stop ( "ControllMysql" );
		if ($this->getDebug ()) {
			Debug::debugDump ( $sql."; sql执行耗时：" . implode ( '', RunTime::spent ( "ControllMysql" ) ) . ' ms' );
		}
		if ($result === false) {
